...
int CCalibration::evaluateCalibrationResult(vector<vector<Point3f>> objectPoints, vector<vector<Point2f>> cornerSquare, vector<Vec3d> _rvec,
											vector<Vec3d> _tvec, Mat _K, Mat _D, int count)
{
	/**
	 * @brief 估计重投影误差，评价标定结果
	 * param1 角点的三维点坐标
	 * param2 角点的二维点坐标
	 * param4 旋转矩阵R
	 * param5 平移矩阵T
	...
	for (int i = 0; i< count; i++)
	{
		...
		//计算三维点投影到二维平面上的坐标（已知内参和外参）
		projectPoints(tempPointSet, _rvec[i], _tvec[i], _K, _D, proImgPoints);
		...
		fout << "第" << i << "幅图像的平均重投影误差：" << sum/tempImgPoint.size() << "像素" << endl;
	}
	fout.close();
	return 0;
}
...
void CCalibration::calibProcess()
{
	...
	evaluateCalibrationResult(object_points, corner2d, rotation, translation,
							  camK, camDiscoeff, successImgNum);
